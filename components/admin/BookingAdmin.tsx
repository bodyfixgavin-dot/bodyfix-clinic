...
export function BookingAdmin() {
  ...
  const loadAdminData = useCallback(async (previewMode: boolean) => {
    if (previewMode) {
  ...
  useEffect(() => {
    async function initializeBooking() {
      ...
      const previewMode = Boolean(data.bypassMode);
      setBypassMode(previewMode);
      setLocalMode(previewMode);
      await loadAdminData(previewMode);
    }
    initializeBooking();
  ...
  return (
    <main className="bf-container bf-admin-mobile-safe">
      <section className="bf-hero">
        ...
        <p className="bf-kicker">BOOKING OPERATIONS</p>
        <h1>預約管理</h1>
        <p className="bf-subtitle">管理可約時段、預約申請、確認、取消與目前預約狀態。</p>
        <nav className="clinic-nav clinic-nav-compact" aria-label="預約管理快速導覽">
      ...
      {bypassMode && (
        <div className="bf-notice bf-admin-notice">Preview Local Mode｜已建立安全管理 session；預約資料只存在此瀏覽器，不會寫入正式資料庫。</div>
      )}
      ...
      <section className="bf-card bf-section-gap">
        <p className="bf-admin-entry-eyebrow">01</p><h2 className="bf-section-title">新增可約時段</h2>
      ...
      <section className="bf-card bf-section-gap">
        <p className="bf-admin-entry-eyebrow">02</p><h2 className="bf-section-title">預約申請</h2>
      ...
      <section className="bf-card bf-section-gap">
        <p className="bf-admin-entry-eyebrow">03</p><h2 className="bf-section-title">所有時段</h2>
