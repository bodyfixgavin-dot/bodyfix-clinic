...
const navItems = [
  { label: "身體判讀", href: "/learner/field", activePaths: ["/learner", "/learner/field"] },
  { label: "學習內容", href: "/learner/atlas", activePaths: ["/learner/atlas"] },
  { label: "學習階段", href: "/learner/passport", activePaths: ["/learner/passport"] },
];
...
export function LearnerShell({ children }: { children: ReactNode }) {
  ...
  return <main className="learner-page"><div className="learner-shell"><nav className="learner-nav"><Link className="portal-wordmark" href="/learner"><span>BF</span><strong>BODYFIX 身體判讀</strong></Link><div>{navItems.map((item) => <Link className={isActivePath(pathname, item.activePaths) ? "is-active" : undefined} href={item.href} key={item.href} aria-current={isActivePath(pathname, item.activePaths) ? "page" : undefined}>{item.label}</Link>)}</div></nav>{children}<footer className="learner-footer"><Link href="/">返回 BodyFix OS</Link><span>原型階段 · 各自成樹，彼此成林；地下共生，向光循環。</span></footer></div></main>;
}
